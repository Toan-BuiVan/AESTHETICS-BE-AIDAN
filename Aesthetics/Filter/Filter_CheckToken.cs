...
namespace ASP_NetCore_Aesthetics.Filter
{
	public class Filter_CheckToken : IAsyncAuthorizationFilter
	{
		...
		public Filter_CheckToken(IAccountRepository accountRepository
		...
		public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
		{
			...
			if (userDetail.TokenExpired < DateTime.Now)
			{
			...
			var user = await _accountRepository.GetUser_ByUserName(userName);

			if (user == null || user.RefreshToken != refreshToken || user.TokenExpired <= DateTime.Now)
			{
				SetUnauthorizedResponse(context, "Token không hợp lệ!");
				return;
			}
			...
			var newToken = await _tokenService.CreateToken(principal.Claims.ToList());
			var newRefreshToken = await _tokenService.GenerateRefreshToken();

			// Cập nhật token mới vào DB
			_ = int.TryParse(_configuration["JWT:RefreshTokenValidityInDays"], out int refreshTokenValidityInDays);
			await _accountRepository.UserUpdate_RefeshToken(user.Id, newRefreshToken, DateTime.Now.AddDays(refreshTokenValidityInDays));
